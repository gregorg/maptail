...
maptail.on('message', function (message) {
  ...
  if (!maptail.config.onlyLookups || geo.ll) maptail.emit('geoip', message)
})
...
maptail.attach = function (app) {
  ...
  maptail.on('geoip', function (geo) {
    ...
    if (!maptail.config.quiet) console.log("Emit a packet of " + maptail.buffer.length + " objects, " + (maptail.config.noduplicates ? maptail.duplicates + " duplicate IP, " : '') + maptail.discards + " discarded")
