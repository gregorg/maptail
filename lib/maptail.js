...
maptail.on('json', function (message) {
  maptail.emit('geoip', message)
})
...
maptail.attach = function (app) {
  ...
  maptail.on('geoip', function (geo) {
    ...
    if (Date.now() - before > maptail.config.bufferTime && maptail.buffer.length) {
	  ...
	  console.log("Emit a packet of " + maptail.buffer.length + " objects, " + maptail.duplicates + " duplicate IP, " + maptail.discards + " discarded")

      before = Date.now()
      if (maptail.discards >= maptail.config.bufferMax/10)
      {
        console.warn('WARNING: ' + maptail.discards + ' packets discard')
        maptail.discards = 0
      }
      maptail.buffer = []
      maptail.ipbuffer = []
